...
function calcShowText1 (roomList) {
    const a = roomList[0].adult; // 每間房大人人數
    const b = roomList[0].childrenWithBed.length; // 每間房小孩佔床人數
    const c = roomList[0].childrenNoBed.length; // 每間房小孩不佔床人數
    const total = a + b + c;
    return `${total}人`;
}
            ...
            <div className="child_row">
                <div className="room_list_row">
                    <div className="child_count_control">
                        <span className="row_title">{noHotel === 0 ? '小孩佔床' : '小孩'}</span>
                        <IntGpct
                            max={3}
                            min={0}
                            count={childBedCount}
